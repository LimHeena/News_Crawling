...
class Crawler:
    ...
    def naver_crawler(self):
        ...
        for sub_num in sub_category_num:
            ...
            print('--------------------------------------------------------------')
            print(f'서브카테고리({self.SUB_CATEGORY_DICT[sub_num]}) 하나 크롤링 중~')
            ...
            df.sort_values('writed_at') #정렬
            
            self.SUB_CATEGORY_URL[sub_num] = df.iloc[-1]['url']
